...
function getPatientIDs(uint256[] memory _patientIDs,uint256 _studyID,bytes32[] memory categorysfromquery,bytes32[] memory elementsfromquery, int256 _endTime )public view returns(uint256[] memory ){
    ...
    for(uint i=0;i<length;i++)
        {  
           patient=dataBase[_studyID][_patientIDs[i]];
        if(patient.consent.length>0){
           if(_patientIDs[i]==0)break;
        ...
        if(patient.consent[0].recordTime<=uint256(_endTime)){ 
...
function extractChoicesFromInput(string[] memory inputelement,string[] memory inputCategory)public pure returns(bytes32[]memory,bytes32[]memory){
    ...
    */
    ...
    function handleelementSharingChoices(string[] calldata elementSharingChoices) public  returns (bytes32) {
    bytes32 result;
    bytes32 shortnumber;
    bytes memory q;
    ...
    function asciiToInt(string memory _asciiString) public pure returns (uint) {
    
    bytes memory asciiBytes = bytes(_asciiString);
    uint result = 0;
    uint digit;
    
    for(uint i = 0; i < asciiBytes.length; i++) {
      digit = uint8(asciiBytes[i]) - 48;
